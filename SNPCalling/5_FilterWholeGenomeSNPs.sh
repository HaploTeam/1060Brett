...
mkdir $WORKDIR/01_WholeGenomeSNPs
cd $WORKDIR/01_WholeGenomeSNPs
...
bcftools query -l $DATADIR/All2nOnly-All2nOnly-full.ANAFAC.SNPs.Novar.GQ20.DP5.cov5pto95p.vcf.gz | cut -d '/' -f 7 | cut -d '-' -f 1 > NewSamples.txt

# Get list of non admixed samples
cat $DATADIR/Clusters.txt | cut -f 1 > NonAdmixedSamples.txt

bcftools reheader --sample NewSamples.txt $DATADIR/All2nOnly-All2nOnly-full.ANAFAC.SNPs.Novar.GQ20.DP5.cov5pto95p.vcf.gz | \
	bcftools view -i 'F_MISSING<0.1' | \
	bcftools view --samples-file NonAdmixedSamples.txt | \
	bcftools +fill-tags | \
	bcftools view --threads 4 -Oz -o All2nOnly-All2nOnly-full.ANAFAC.SNPs.Novar.GQ20.DP5.cov5pto95p.Miss10-Mind20.NonAdmixed.vcf.gz
